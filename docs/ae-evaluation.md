...
For SSH evaluation on the project workstation, the versioned gateway in
[`ae/`](../ae/README.md) accepts only `run offline`, `status <run-id>`,
...
project operator provides the SSH address and authorized key out of band. The
gateway, frozen checkout, deployment configuration and results directory may
be in different filesystem locations; the config uses absolute paths. The
gateway code and its unit tests are public, while keys, real config and run
