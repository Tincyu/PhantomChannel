#!/usr/bin/env python3
"""Build and flash the independent phone-facing PIP HRS peripheral image."""
...
SAMPLE_DIR = SDK_ROOT / "zephyr/samples/bluetooth/phantomchannel_pip_hrs_peripheral"
BUILD_DIR = PROJECT_ROOT / "artifacts/firmware/phantomchannel_pip_hrs_peripheral"
...
def build_with_settings(covert_len: int, interval_ms: int) -> Path:
    ...
    with tempfile.NamedTemporaryFile(
        mode="w",
        encoding="utf-8",
        prefix="phantomchannel_pip_hrs_peripheral_",
        suffix=".conf",
        dir=overlay_dir,
        delete=False,
    ) as overlay_file:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help=f"heart-rate notification interval ({MIN_INTERVAL_MS}..{MAX_INTERVAL_MS} ms)",
    )
    parser.add_argument("--build", action="store_true", help="rebuild before flashing")
    parser.add_argument(
        "--build-only", action="store_true", help="build without flashing"
    )
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="verify the exact board and existing image without building or flashing",
    )
    ...
    if args.verify_only and (parameterized_build or args.build or args.build_only):
        raise SystemExit(
            "--verify-only cannot be combined with parameters, --build, or --build-only"
        )
    ...
    if args.build_only:
        ...
        print(
            f"PIP HRS peripheral built: {FIRMWARE}; "
    ...
    if args.verify_only:
        print(f"PIP HRS peripheral ready: {args.serial_number} {FIRMWARE}")
        return 0
    flash_image(FIRMWARE, args.serial_number, BOARD_VERSION)
    print(
        f"PIP HRS peripheral flashed: {args.serial_number}; "
        f"covert_len={covert_len} B, interval={interval_ms} ms"
    )
    return 0
